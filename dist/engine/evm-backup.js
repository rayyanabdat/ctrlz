...
import { createAbortedResult, shouldAbortScan } from "./analysis-types.js";
...
export async function scanEvmContract(address, chainKey = "ethereum") {
    ...
    try {
        ...
        scanResult = createAbortedResult("RPC_FAILURE", "INITIALIZATION", error instanceof Error ? error.message : "Unknown error", { contractAddress: address, scanId, timestamp: Date.now() });
    }
    // Display results
    displayScanResult(scanResult);
}
async function performScan(address, chainKey, scanId) {
    ...
    try {
        ...
        if (shouldAbortScan(securityResult) || shouldAbortScan(ownershipResult)) {
            const reason = securityResult.risk === "CRITICAL" ? "CRITICAL_RISK" : "HIGH_RISK";
            return {
                ...scanResult,
                aborted: {
                    reason,
        ...
        return createAbortedResult("RPC_FAILURE", "SCAN_EXECUTION", error instanceof Error ? error.message : "Unknown error", scanResult);
    }
}
async function analyzeToken(rpcPool, address, chain) {
    const startTime = Date.now();
    const result = {
        name: null,
        symbol: null,
    ...
    try {
        ...
        const codeResult = await rpcPool.call("eth_getCode", [address, "latest"], `code_${address}`);
...
async function analyzeOwnership(rpcPool, address, chain) {
    const startTime = Date.now();
    const result = {
        ownerAddress: null,
        ownerType: null,
    ...
    try {
        ...
        const storageResult = await rpcPool.call("eth_getStorageAt", [address, IMPLEMENTATION_SLOT, "latest"], `proxy_${address}`);
        ...
        for (const pattern of ownershipPatterns) {
            const ownerResult = await rpcPool.call("eth_call", [{ to: address, data: pattern.selector }], `owner_${address}_${pattern.name}`);
            if (ownerResult.success && ownerResult.data && ownerResult.data.length >= 66) {
                ...
                if (isAddress(ownerAddr)) {
                    ...
                    if (result.ownerAddress === zeroAddress) {
                        ...
                        const ownerCodeResult = await rpcPool.call("eth_getCode", [result.ownerAddress, "latest"], `owner_code_${result.ownerAddress}`);
...
async function analyzeSecurity(rpcPool, address, chain) {
    const startTime = Date.now();
    const result = {
        functions: {
            mint: false,
    ...
    try {
        ...
        const functionChecks = [
            { selector: "0x40c10f19", name: "mint", key: "mint" },
            { selector: "0x42966c68", name: "burn", key: "burn" },
            { selector: "0x8456cb59", name: "pause", key: "pause" },
            { selector: "0xf9f92be4", name: "blacklist", key: "blacklist" },
            { selector: "0x69fe0e2d", name: "setFee", key: "setFee" },
            { selector: "0xb782ac4c", name: "rescue", key: "rescue" }
        ];
        const functionResults = await Promise.allSettled(functionChecks.map(fn => rpcPool.call("eth_call", [{ to: address, data: fn.selector }], `func_${address}_${fn.name}`)));
...
async function analyzeHoldersEnhanced(rpcPool, address, chain, ownerAddress) {
    const startTime = Date.now();
    const result = {
        topHolders: [],
        distribution: {
    ...
    try {
        ...
        const supplyResult = await rpcPool.call("eth_call", [{ to: address, data: "0x18160ddd" }], // totalSupply()
        `supply_${address}`);
        ...
        const balanceChecks = await Promise.allSettled(addressesToCheck.map(item => rpcPool.call("eth_call", [{ to: address, data: "0x70a08231" + item.addr.slice(2).padStart(64, '0') }], // balanceOf(address)
        `balance_${address}_${item.addr}`).then(result => ({ ...item, result }))));
        let totalChecked = 0n;
        let knownHolders = 0;
...
function calculateFinalRisk(scanResult) {
    const risks = [
...
function calculateFinalConfidence(scanResult) {
    const confidences = [
    ...
    if (confidences.includes("UNVERIFIABLE"))
        return "UNVERIFIABLE";
    // If all are VERIFIED, final is VERIFIED
    if (confidences.every(c => c === "VERIFIED"))
        return "VERIFIED";
    return "PARTIAL";
}
...
function displayAnalysisSection(title, analysis) {
    ...
    if (analysis.evidence.length > 0) {
        ...
        analysis.evidence.slice(0, 3).forEach((evidence) => {
...
//# sourceMappingURL=evm-backup.js.map