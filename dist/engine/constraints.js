...
async function readContractValue(client, address, abi, functionName) {
    try {
...
export async function analyzeConstraints(client, tokenAddress, ownerAddress, ownerType) {
    const result = {
        hasCooldown: false,
        hasBlacklist: false,
        hasWhitelist: false,
    ...
    if (maxTxAmount || maxWallet || maxTxAlt || maxWalletAlt) {
        ...
        const limits = [];
        if (maxTxAmount || maxTxAlt)
            limits.push("max transaction");
        if (maxWallet || maxWalletAlt)
            limits.push("max wallet");
    ...
    if (buyTax !== null || sellTax !== null || totalFees !== null) {
        ...
        const taxDetails = [];
        if (buyTax !== null)
            taxDetails.push(`buy: ${buyTax}%`);
        if (sellTax !== null)
            taxDetails.push(`sell: ${sellTax}%`);
    ...
    if (result.hasCooldown)
        riskFactors += 1;
    if (result.hasAntiWhale)
        riskFactors += 1;
    if (result.hasWhitelist)
        riskFactors += 1;
    if (result.hasTax && !result.hasDynamicTax)
        riskFactors += 1;
    ...
    if (result.hasAntiWhale)
        riskFactors += 1;
    if (result.hasTax && !result.hasDynamicTax)
        riskFactors += 0; // Fixed tax is not risky
