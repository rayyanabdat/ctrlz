...
import { analyzeLiquidity, LiquidityResult } from "./liquidity.js";
import { analyzeConstraints, ConstraintResult } from "./constraints.js";
import { analyzeHolders, HolderResult } from "./holders.js";
import { calculateFinalScore, ScoreResult, RiskLevel, LiquidityRiskBreakdown } from "./scoring.js";
import { getTokenIdentity, TokenIdentity } from "./identity.js";
import { analyzeContext, ContextResult } from "./context.js";
...
export async function scanEvmContract(address: string, chainKey: string = "ethereum"): Promise<void> {
  ...
  const client = createPublicClient({
    chain: undefined,
    transport: http(chain.rpcUrl, { timeout: 20_000 })
  });
  ...
  const { isProxy, implementationAddress, ownerAddress, ownerType, detectedFunctions, riskLevel, logicFacts, logicEvidence } = 
    await analyzeLogicRisk(client, address, chainKey);
  ...
  const contextResult = await analyzeContext(
    client, address, tokenIdentity, liquidityResult, constraintResult, holderResult, isProxy, ownerType
  );
  ...
  const scoreResult = calculateFinalScore({
    ...
    constraintRisk: constraintResult.risk as RiskLevel,
...
interface LogicRiskResult {
  isProxy: boolean;
  implementationAddress: string | null;
  ownerAddress: string | null;
  ownerType: "EOA" | "CONTRACT" | "ZERO_ADDRESS" | "NOT_FOUND";
  detectedFunctions: Record<string, string[]>;
  riskLevel: RiskLevel;
  logicFacts: string[];
  logicEvidence: string[];
}

async function analyzeLogicRisk(client: any, address: string, chainKey: string): Promise<LogicRiskResult> {
  const explorer = getExplorerUrl(chainKey);
  
  let isProxy = false;
  let implementationAddress: string | null = null;
  let ownerAddress: string | null = null;
  let ownerType: "EOA" | "CONTRACT" | "ZERO_ADDRESS" | "NOT_FOUND" = "NOT_FOUND";
  const logicFacts: string[] = [];
  const logicEvidence: string[] = [];
  const detectedFunctions: Record<string, string[]> = {};
  ...
  try {
    ...
    if (owner && isAddress(owner)) {
      ...
      if (ownerAddress === zeroAddress) {
        ...
        const code = await client.getCode({ address: ownerAddress as `0x${string}` });
