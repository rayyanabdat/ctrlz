...
const STABLECOIN_DECIMALS: Record<string, { symbol: string; decimals: number }> = {
  // Ethereum
...
function getDexUrl(dexName: string, chainKey: string, pairAddress: string): string {
...
export async function analyzeLiquidity(
  ...
  if (result.pools.length > 0) {
    ...
    const sorted = [...result.pools].sort((a, b) => {
      if (a.depthVerifiable && !b.depthVerifiable) return -1;
      if (!a.depthVerifiable && b.depthVerifiable) return 1;
...
function calculateLiquidityRisks(result: LiquidityResult): LiquidityRiskBreakdown {
  let controlRisk: RiskLevel;
  if (!result.found) {
    controlRisk = "HIGH";
  ...
  let verifiabilityRisk: RiskLevel;
  if (!result.found) {
...
async function getTokenDecimals(client: PublicClient, tokenAddress: string): Promise<number> {
  try {
...
async function checkV2Pair(
  ...
  try {
    const pairAddress = await client.readContract({
      ...
      args: [tokenAddress as `0x${string}`, quoteToken as `0x${string}`]
    });
    ...
    let estimatedDepthUsd: number | null = null;
...
async function checkV3Pool(
  ...
  try {
    const poolAddress = await client.readContract({
      ...
      args: [tokenAddress as `0x${string}`, quoteToken as `0x${string}`, fee]
    });
...
async function detectV4Interaction(
  client: PublicClient,
  tokenAddress: string,
  ...
  try {
    const code = await client.getCode({ address: poolManagerAddress as `0x${string}` });
...
async function checkLpProtection(
  client: PublicClient,
  pairAddress: string,
  chainKey: string
): Promise<{ isBurned: boolean; isLocked: boolean; burnPercent: number; lockPercent: number }> {
  ...
  try {
    ...
    if (totalSupply === 0n) return protection;
