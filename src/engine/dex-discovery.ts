...
import { AnalysisEvidence, ConfidenceLevel, LiquidityInfo } from "./analysis-types.js";
...
export class DexDiscovery {
  ...
  async discoverLiquidity(tokenAddress: string): Promise<LiquidityInfo> {
    ...
    try {
      ...
      if (uniquePools.length > 0) {
        ...
        const lockResults = await Promise.allSettled(
          uniquePools.slice(0, 3).map(pool => this.checkLiquidityLock(pool.pairAddress))
        );
   ...
   * LAYER 1: Direct pair discovery from known factories
   */
  private async discoverDirectPairs(tokenAddress: string): Promise<LiquidityInfo["pools"]> {
    const pools: LiquidityInfo["pools"] = [];
    ...
    if (v2Pools.status === "fulfilled") pools.push(...v2Pools.value);
    if (v3Pools.status === "fulfilled") pools.push(...v3Pools.value);
   ...
   */
  private async discoverViaEvents(tokenAddress: string): Promise<LiquidityInfo["pools"]> {
    const pools: LiquidityInfo["pools"] = [];
    ...
    try {
      ...
      if (!viemChain) return pools;
      
      const rpcUrl = this.getRpcUrl();
      if (!rpcUrl) return pools;
      ...
      const factories = DEX_FACTORIES[this.chainKey as keyof typeof DEX_FACTORIES] || {};
      
      for (const [factoryAddress, factoryInfo] of Object.entries(factories)) {
        if (factoryInfo.type !== "UNISWAP_V2") continue;
        ...
        try {
          ...
          const logs0 = await client.getLogs({
            ...
            args: { token0: tokenAddress as `0x${string}` },
            fromBlock: 'earliest',
            toBlock: 'latest'
          });
          ...
          const logs1 = await client.getLogs({
            address: factoryAddress as `0x${string}`,
            event: parseAbiItem('event PairCreated(address indexed token0, address indexed token1, address pair, uint256)'),
            args: { token1: tokenAddress as `0x${string}` },
            fromBlock: 'earliest',
            toBlock: 'latest'
          });
          ...
          for (const log of allLogs) {
            ...
            const args = log.args as { token0?: `0x${string}`; token1?: `0x${string}`; pair?: `0x${string}` };
            if (args.pair) {
   ...
   */
  private async discoverViaPatterns(tokenAddress: string): Promise<LiquidityInfo["pools"]> {
    // This layer tries common pair computation patterns
    // even for DEXes we don't have factory addresses for
    return [];
  }

  private deduplicatePools(pools: LiquidityInfo["pools"]): LiquidityInfo["pools"] {
  ...
  private async discoverUniswapV2Pairs(tokenAddress: string) {
    const pools = [];
    const factories = DEX_FACTORIES[this.chainKey as keyof typeof DEX_FACTORIES] || {};
    ...
    for (const [factoryAddress, factoryInfo] of Object.entries(factories)) {
      ...
      try {
        ...
        for (const pairedToken of commonTokens) {
          const pairResult = await this.rpcPool.call(
            "eth_call",
            [{
              to: factoryAddress,
  ...
  private async discoverUniswapV3Pools(tokenAddress: string): Promise<LiquidityInfo["pools"]> {
    const pools: LiquidityInfo["pools"] = [];
    const factories = DEX_FACTORIES[this.chainKey as keyof typeof DEX_FACTORIES] || {};
    
    for (const [factoryAddress, factoryInfo] of Object.entries(factories)) {
      if (factoryInfo.type !== "UNISWAP_V3") continue;
  ...
  private async discoverCurvePools(tokenAddress: string): Promise<LiquidityInfo["pools"]> {
    // Curve pool discovery would require knowledge of Curve registry
    // This is a placeholder for future implementation
    return [];
  }

  private async discoverBalancerPools(tokenAddress: string): Promise<LiquidityInfo["pools"]> {
    // Balancer pool discovery would require vault interaction
    // This is a placeholder for future implementation
    return [];
  }

  private async getUniswapV2Reserves(pairAddress: string) {
    const result = await this.rpcPool.call(
      "eth_call",
      [{ to: pairAddress, data: POOL_SIGNATURES.getReserves }],
    ...
    return {
      token0: "0",
      token1: "0",
      confidence: "UNVERIFIABLE" as ConfidenceLevel
    };
  }

  private async checkLiquidityLock(pairAddress: string): Promise<boolean> {
  ...
  private getCommonTokensForChain(): string[] {
    const commonTokens = {
      ethereum: [
    ...
    return commonTokens[this.chainKey as keyof typeof commonTokens] || commonTokens.ethereum;
  }
   ...
   */
  private async discoverKnownPairs(tokenAddress: string): Promise<LiquidityInfo["pools"]> {
    const pools: LiquidityInfo["pools"] = [];
    const tokenLower = tokenAddress.toLowerCase();
    
    // Known major token pairs
    const knownPairs: Record<string, Record<string, string>> = {
      ethereum: {
        // SHIB pairs
    ...
    const chainPairs = knownPairs[this.chainKey as keyof typeof knownPairs] || {};
