...
function countRiskPenalty(risk: RiskLevel): number {
  switch (risk) {
    case "LOW": return 0;
...
function isMediumRisk(risk: RiskLevel): boolean {
  return risk === "MEDIUM";
}

function isUnknown(risk: RiskLevel): boolean {
  return risk === "UNKNOWN" || risk === "UNVERIFIABLE";
}

function getBand(score: number): string {
  if (score >= 90) return "STRONG CONFIDENCE";
...
export function calculateFinalScore(input: ScoreInput): ScoreResult {
  ...
  if (logicPenalty !== 0) {
    ...
    if (isHighRisk(input.logicRisk)) riskFactors.push("Critical/High logic risk detected");
    if (isMediumRisk(input.logicRisk)) riskFactors.push("Moderate logic risk detected");
    if (isUnknown(input.logicRisk)) riskFactors.push("Logic risk could not be fully verified");
  }
  ...
  if (lpControlPenalty !== 0) {
    ...
    if (isHighRisk(input.liquidityRisk.controlRisk)) riskFactors.push("LP tokens not protected (burned/locked)");
  }
  ...
  if (lpDepthPenalty !== 0) {
    ...
    if (isHighRisk(input.liquidityRisk.depthRisk)) riskFactors.push("Insufficient liquidity depth (<$1,000)");
    if (isMediumRisk(input.liquidityRisk.depthRisk)) riskFactors.push("Low liquidity depth (<$10,000)");
  }
  ...
  if (lpVerifiabilityPenalty !== 0) {
    ...
    if (isUnknown(input.liquidityRisk.verifiabilityRisk)) riskFactors.push("Liquidity depth could not be verified");
  }
  ...
  if (constraintPenalty !== 0) {
    ...
    if (isHighRisk(input.constraintRisk)) riskFactors.push("Restrictive trading controls detected");
    if (isMediumRisk(input.constraintRisk)) riskFactors.push("Trading constraints detected");
  }
  ...
  if (holderPenalty !== 0) {
    ...
    if (isHighRisk(input.holderRisk)) riskFactors.push("High holder concentration risk");
    if (isMediumRisk(input.holderRisk)) riskFactors.push("Moderate holder concentration");
    if (isUnknown(input.holderRisk)) riskFactors.push("Holder distribution could not be fully verified");
  }
