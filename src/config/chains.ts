...
export interface ChainConfig {
  ...
  name: string;
  chainId: number;
  rpcUrl: string; // Primary RPC (backward compatibility)
  rpcUrls?: string[]; // Fallback RPCs (backward compatibility)
  rpcPool: RpcEndpoint[]; // New tiered RPC system
  wrappedNative: string;
  stablecoins: string[];
}

function resolveRpc(envKey: string, primaryFallback: string, secondaryFallback: string): string {
  const envRpc = process.env[envKey];
  if (envRpc && envRpc.trim() !== "") return envRpc;
...
export function getChain(chainKey?: unknown): ChainConfig {
